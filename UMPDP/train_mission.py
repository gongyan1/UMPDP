...
# from models.yolo_test_mission import Model
from models.yolo_test_backbone import Model
from utils.autoanchor import check_anchors
# from utils.datasets import create_dataloader, create_dataloader_rgb_ir # create_dual_dataloader,
from utils.datasets_sk import create_dataloader, create_dataloader_rgb_ir
from utils.general import labels_to_class_weights, increment_path, labels_to_image_weights, init_seeds, \
    fitness, strip_optimizer, get_latest_run, check_dataset, check_file, check_git_status, check_img_size, \
    check_requirements, print_mutation, set_logging, one_cycle, colorstr
from utils.google_utils import attempt_download
# from utils.loss import ComputeLoss
from utils.loss_sk import ComputeLoss
...
def train(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        optimizer.zero_grad()
        for i, (imgs, targets, paths, _, day_or_night) in pbar:  # batch -------------------------------------------------------------
...
#                 pred = model(imgs_rgb, imgs_ir)  # forward
                pred, day_or_night_pred = model(imgs_rgb, imgs_ir)
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                wandb_logger.current_epoch = epoch + 1
                results, maps, times = test_mission.test(data_dict,
                                                 batch_size=batch_size * 2,
                                                 imgsz=imgsz_test,
                                                 model=ema.ema,
                                                 single_cls=opt.single_cls,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='./yolov5s_TN_DMAF_cas_sk.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='./data/kaist_thermal_RGB_sk_7060.yaml', help='data.yaml path')
...
# python train_mission.py --img 640 --batch 16 --epochs 100 --data data/kaist_thermal_RGB_sk_7060.yaml --cfg models/yolov5_TN_DMAF_att.yaml --weights ./yolov5s.pt

# python train_mission.py --img 640 --batch 16 --epochs 100 --data data/cvc14_thermal_RGB.yaml --cfg models/yolov5_TN_DMAF_att.yaml --weights ./yolov5s.pt
