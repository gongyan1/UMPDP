...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes, day_or_night) in enumerate(tqdm(dataloader, desc=s)):
    # for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(testloader, desc=s)):
        ...
        with torch.no_grad():
            # Run model
            t = time_synchronized()
            out_tmp, day_or_night_pred = model(img_rgb, img_ir, augment=augment)  # inference and training outputs
            out, train_out = out_tmp
            t0 += time_synchronized() - t

            # Compute loss
            if compute_loss:
                loss += compute_loss([x.float() for x in train_out], targets, day_or_night.to(device), day_or_night_pred.to(device))[1][:3]  # box, obj, cls
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='./runs/train/exp68/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='./data/kaist_thermal_RGB_sk_7060.yaml', help='*.data path')
...
# python test_multi.py  --task test --img-size 640 --batch-size 16 --data data/kaist_thermal_RGB_cvc14.yaml --weights runs/train/CVC-14/exp8/weights/best.pt