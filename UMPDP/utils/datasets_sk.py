...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if filename.split('set')[1][:2] in ['00','01','02','06','07','08']:
            print('day+++++++++++++++++++++++++++++++++++++++++++++++++++++')
            day_or_night=np.array([1])
        else:
            day_or_night=np.array([0])

        return torch.from_numpy(img), labels_out, self.img_files[index], shapes, torch.from_numpy(day_or_night)

    @staticmethod
    def collate_fn(batch):
        img, label, path, shapes, day_or_night = zip(*batch)  # transposed
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
        return torch.stack(img, 0), torch.cat(label, 0), path, shapes, torch.stack(day_or_night,0)
...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(self, path_rgb, path_ir, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            for p_rgb in path_rgb if isinstance(path_rgb, list) else [path_rgb]:
                ...
                # -----------------------------  ir   -----------------------------ssh -p 53491 root@tx-nat.videojj.com
        ...
        # Check cache
        # Check rgb cache
        self.label_files_rgb = img2label_paths(self.img_files_ir)  # labels
        # print(self.label_files)
    ...
    def __getitem__(self, index):
        ...
        filename=self.img_files_rgb[index]

        if "CVC" in filename:
            filename_split = filename.split('/')
            train_or_test = filename_split[-3]
            _is_exist = filename_split[-1].split('.')[0][-1]

            if train_or_test == "Train" and _is_exist == "_":
                day_or_night = np.array([1])  # day
            elif train_or_test == "Test" and _is_exist != "_":
                day_or_night = np.array([1])  # day
            else:
                day_or_night = np.array([0])  # day
            # print(filename, day_or_night)
        ...
        return torch.from_numpy(img_all), labels_out, self.img_files_rgb[index], shapes, torch.from_numpy(day_or_night)
