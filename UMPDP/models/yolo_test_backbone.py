...
#         self.l0_fusion = ASFFV5(level=0, rfb=False)
#         self.l1_fusion = ASFFV5(level=1, rfb=False)
#         self.l2_fusion = ASFFV5(level=2, rfb=False)
...
class Model(nn.Module):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        self.FC_initial1 = nn.Sequential(
...
#             nn.ReLU(),
        ...
        self.condition11 = nn.Sequential(
            nn.Conv2d(128, 128, 1, 1),
            nn.BatchNorm2d(128),
            nn.ReLU(),
        )
...
#             nn.ReLU(),
        ...
        self.condition21 = nn.Sequential(
            nn.Conv2d(256, 256, 1, 1),
            nn.BatchNorm2d(256),
            nn.ReLU(),
        )
...
#             nn.ReLU(),
        ...
        self.condition31 = nn.Sequential(
            nn.Conv2d(512, 512, 1, 1),
            nn.BatchNorm2d(512),
            nn.ReLU(),
        )
...
#                 print(FC_input.shape,'+++++++++++++++++++++++++++++++++++++++++')
                FC_initial1 = self.FC_initial1(FC_input)
                
            

            if m.i == 9: 
                gamma1 = self.gamma1(FC_initial1)
                beta1 = self.beta1(FC_initial1)
                x = self.condition1(((1 - gamma1) * x + beta1)+ x) 
            if m.i ==11: 

                x = self.condition11((gamma1 * x + beta1) + x)
                
                
                
            if m.i == 16:
                gamma2 = self.gamma2(FC_initial1)
                beta2 = self.beta2(FC_initial1)
                x = self.condition2(((1 - gamma2) * x + beta2)+ x) 
            if m.i == 18:
               
                x = self.condition21((gamma2 * x + beta2)+ x) 
                
                
                
            if m.i == 23:
                gamma3 = self.gamma3(FC_initial1)
                beta3 = self.beta3(FC_initial1)
                x = self.condition3(((1 - gamma3) * x + beta3)+ x) 
            if m.i == 26:
                x = self.condition31((gamma3 * x + beta3)+ x) 
        ...
        fc_mid = FC_initial1.view(FC_initial1.size()[0], -1)
        return x, self.out_fc(fc_mid)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='yolov5s_add_fusion_sk.yaml', help='model.yaml')
