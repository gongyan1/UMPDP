...
class KAISTPedEval(COCOeval):
    ...
    def draw_figure(ax, eval_results, methods, colors):
        ...
        legend.get_texts()[-1].set_font_properties(font)
        ...
        ax.set_ylim(0.01, 1)
        ax.set_xlim(2e-4, 50)
        # ax.set_ylabel('Log-average miss rate')
        # ax.set_xlabel('False positive per image')
...
def evaluate(test_annotation_file: str, user_submission_file: str, phase_codename: str = 'Multispectral'):
    ...
    eval_result['all'].evaluate(id)
    eval_result['all'].accumulate()
    MR_all = eval_result['all'].summarize(id)

    eval_result['day'].params.imgIds = imgIds[:1455]
    eval_result['day'].evaluate(id)
    eval_result['day'].accumulate()
    MR_day = eval_result['day'].summarize(id)

    eval_result['night'].params.imgIds = imgIds[1455:]
    eval_result['night'].evaluate(id)
    eval_result['night'].accumulate()
    MR_night = eval_result['night'].summarize(id)
